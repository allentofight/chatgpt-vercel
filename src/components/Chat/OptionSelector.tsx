...
const options = [
    {
        title: 'GPT-3.5',
        description:
            'GPT-3.5，仅对充值用户提供。此模型支持 4k token（大约 2000 字）的上下文会话',
    },
    {
        title: 'New Bing',
        description:
            'New Bing 是微软最新推出的搜索引擎，底层基于 OpenAI ChatGPT 4 模型，可以让你和它进行智能对话，获取网上最新信息',
    },
    {
        title: 'GPT-4',
        description:
            '当今世界最强人工智能模型，此模型支持 8k token（大约 4000 字）的上下文会话',
    },
...
function OptionSelector() {
    ...
    onMount(() => {
        const selectOptionHandler = (
            event: CustomEvent<{ index: number; disabled: boolean }>
        ) => {
                ...
                <div class="relative flex rounded-xl bg-gray-100 p-1 text-gray-900 dark:bg-gray-900 light:bg-[#4181FF30]">
                    <ul class="flex w-full list-none gap-1 sm:w-auto">
                        <Show when={!modelConfirmed() || !isGPT4()}>
                            <li
                                class="group/toggle w-full"
                                onMouseOver={() => handleHover(true, 1)}
                                onMouseOut={() => handleHover(false, 1)}
                                onClick={() => {
                                    setGPT4(false);
                                    const optionSelectedEvent = new CustomEvent(
                                        'optionSelected',
                                        {
                                            detail: {
                                                index: 1,
                            ...
                            >
                                ...
                                >
                                    <div
                                        class={`group/button relative flex w-full items-center justify-center gap-1 rounded-lg py-3 outline-none transition-opacity duration-100 sm:w-auto sm:min-w-[148px] md:gap-2 md:py-2.5 border-black/10 text-gray-500 hover:!opacity-100 ${
                                            !isGPT4()
                                                ? 'border bg-white dark:border-[#4E4F60] dark:bg-gray-700 dark:text-gray-100 shadow-[0_1px_7px_0px_rgba(0,0,0,0.06)]'
                                    ...
                                    >
                                        ...
                                        <span class="truncate text-sm font-medium md:pr-1.5 pr-1.5">
                                            4o mini
                                        </span>
                                    </div>
                                </button>
                        ...
                        <Show when={!modelConfirmed() || isGPT4()}>
                            ...
                            >
                                ...
                                >
                                    <div
                                        class={`relative flex w-full items-center justify-center gap-1 rounded-lg border py-3 outline-none transition-opacity duration-100 sm:w-auto sm:min-w-[148px] md:gap-2 md:py-2.5 border-transparent text-gray-500 ${
                                            isGPT4()
                                                ? 'bg-white dark:border-[#4E4F60] border dark:bg-gray-700 dark:text-gray-100 shadow-[0_1px_7px_0px_rgba(0,0,0,0.06)]'
                                                : 'hover:text-gray-800 hover:dark:text-gray-100'
                                        }  gpt4-svg-target`}
                                    >
                                        <span class="relative max-[370px]:hidden">
                                        ...
                                        <span class="truncate text-sm font-medium md:pr-1.5">
                                            GPT-4o
                                        </span>
                                        <svg
                                            stroke="currentColor"
