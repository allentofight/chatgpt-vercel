...
export default function VipItem(props: { type: VipType }) {
    let vipInfo = {
        [VipType.MONTH]: {
    ...
    const handleBuy = () => {
        let selectedItems = encodeURIComponent(
            JSON.stringify(checkedServices())
        );
        let inviteCode = encodeURIComponent(
            localStorage.getItem('inviteCode') as string
        );
    ...
    return (
        ...
        >
            ...
            <div class="p-6 text text-xs flex items-end">
                ...
                <Show when={calculatePrice().itemsCnt > 1}>
                    <span class="text-sm text-red-500 ml-2 font-bold">
                        {i18n.t('saved')} ¥{calculatePrice().saved.toFixed(1)}
                    </span>
                </Show>
            </div>
            ...
            <div class="py-2">
                ...
                <div class="flex cell justify-between">
                    <div class="text">{i18n.t('creativeDrawing')}</div>
                    <div class="text flex-1 text-right">
                        {i18n.t('unLimitedTimes')}/{i18n.t('monthUnit')}
                    </div>
                </div>
                <div class="flex cell justify-between">
                    <div class="text">直推收益奖励</div>
                    <div class="text flex-1 text-right">10%</div>
                </div>
            </div>
        </div>
    );
