...
export default function VipEnItem(props: {
  type: VipType
}) {
  ...
  const [checkedServices, setCheckedServices] = createSignal({ GPT: true, Midjourney: false });
  ...
  const calculatePrice = () => {
    ...
    if (services.GPT && services.Midjourney) {
      const saved = option.prices.GPT + option.prices.Midjourney - option.prices.mergeALL;
  ...
  const createOrder = async () => {
    try {
      ...
      let data = await paypalCreate(JSON.stringify({
    ...
    <>
      ...
      <div class={`vip-item h-full rounded-xl overflow-hidden ${vipInfo[props.type].divFontColorClass}`}>
        ...
        <div class="mt-2 ml-6 mb-3 text-white">
          <label class="inline-flex items-center w-full"><input type="checkbox" class="form-checkbox w-4 h-4" checked={checkedServices().GPT} onChange={() => handleCheckboxChange('GPT')} /><span class="ml-2">GPT 3.5 & GPT4 &nbsp;&nbsp;$
            {vipInfo[props.type].prices.GPT}
          </span></label>
          <label class="inline-flex items-center w-full"><input type="checkbox" class="form-checkbox w-4 h-4" checked={checkedServices().Midjourney} onChange={() => handleCheckboxChange('Midjourney')} /><span class="ml-2">Midjourney&nbsp;&nbsp;&nbsp;$
