...
import { getRequestImageSize } from '~/utils';
...
export default function Draw(props: { showMoreClick: () => void }) {
    ...
    let previousIndex = -1;
    let [isAssistantHintVisible, setIsAssistantHintVisible] =
        createSignal(false);
    createEffect(() => {
    ...
    createEffect(() => {
        ...
        if (picRatioIndex() > -1) {
            ...
            if (match) {
                if (picRatioIndex() < 5) {
                    textareaRef()!.value = prompt.replace(
                        match[2],
                        ratioInfos[picRatioIndex()]
                    );
                } else {
                    ratioInputChange();
                }
                ...
                if (picRatioIndex() < 5) {
                    textareaRef()!.value =
                        prompt + ' --ar ' + ratioInfos[picRatioIndex()];
                } else {
                    ratioInputChange();
                }
    ...
    function upscaling(command: string) {
        ...
        if (
            message.clickedButtons?.includes(command) &&
            command.includes('U')
        ) {
    ...
    function isPromptValid(prompt: string): boolean {
        const parameterRegex: RegExp =
            /--(aspect|ar|chaos|no|quality|q|repeat|r|seed|stop|stylize|s|tile|v|version|niji|iw|s|style)( (\d{1,10}(:\d{1,10})?|[a-zA-Z]+|\d(\.\d+)?))?/;
        ...
        for (let i = 0; i < words.length; i++) {
            if (words[i].startsWith('--')) {
                ...
                if (
                    ...
                    paramsWithValue.includes(parameter)
                ) {
        ...
        try {
            ...
            if (res?.status !== 'SUCCESS') {
                ...
                if (processQueryCount * 5000 > fiveMinutes) {
                    res.status = 'FAILURE';
                    res.failReason = `${i18n.t('taskTimeOut')}，${i18n.t(
                        'retryHint'
                    )}`;
                }
            }
            ...
            if (res?.status === 'SUCCESS') {
                ...
                processQueryCount = 0;
                clearInterval(queryIntervalId);
                let imageSizeRes = getRequestImageSize(
                    res.imageUrl,
                    '358x358',
                    store.inChina
                );
                ...
                setDrawingProgress(
                    res.progress.length > 0 ? res.progress : '0%'
                );
            }
        } catch (error) {
            setIsMjWorking(false);
    ...
    async function sendPrompt(body: MjSendBody) {
        ...
        queryIntervalId = window.setInterval(
            () => queryDrawingProcess(res.messageId),
            5000
        );
    }
    ...
    * Upbeta: use --upbeta to use an alternative beta upscaler when selecting the U buttons.
    ...
    function fetchMessageList(onComplete?: () => void) {
        let earliestGmtCreate =
            messageList().length > 0
                ? messageList()[messageList().length - 1].gmtCreate
                : '';
        fetchMjMessageList(earliestGmtCreate)
            .then((data) => {
                let result = data.list.map((item) => {
                    ...
                    return {
                        ...
                        clickedButtons: item.clickedEvent
                            ? JSON.parse(item.clickedEvent)
                            : [],
                        type: item.type,
                        errorMessage: item.errorMessage,
                        imageUrl:
                            item.cloudPreviewUrl ?? imageSizeRes.previewUrl,
                ...
                if (activeIndex() === -1 && messageList().length) {
                    ...
                    let errorMessage =
                        messageList()[0].errorMessage?.length ?? 0;
    ...
    const handleScroll = () => {
        ...
        if (isMobile()) {
            if (
                previewListRef()!.scrollLeft + previewListRef()!.clientWidth <
                previewListRef()!.scrollWidth - offset
            ) {
                return;
            }
        } else if (
            previewListRef()!.scrollHeight - previewListRef()!.scrollTop >
            previewListRef()!.clientHeight + offset
        ) {
            return;
        }
    ...
    // 自定义高
    const [ratioHeightRef, setRatioHeightRef] =
    ...
    const ratioInputChange = () => {
        ...
        if (ratioWidth.value && ratioHeight.value) {
            ...
            if (match) {
                ...
                textareaRef()!.value =
                    prompt + ` --ar ${ratioWidth.value}:${ratioHeight.value}`;
            }
        }
    };
    ...
    const handleImageUpload = async (file: File) => {
        ...
        const response = await fetch(
            `https://api.superbed.cc/upload?token=${token}`,
            {
                method: 'POST',
                body: formData,
            }
    ...
    createEffect(() => {
        if (!store.hasAiDrawClicked) {
            ...
            setClassName(
                store.menuTitle === i18n.t('aidraw') ? 'flex' : 'hidden'
            );
        }
    });
            ...
            <div class={`_draw flex-1 w-full overflow-hidden ${className()}`}>
                <div class="left-container left flex flex-col w-56">
                    <div class="left-top h-14 flex items-center justify-between px-4 cursor-pointer">
                        <span class="span text-sm">
                            {i18n.t('drawingRecords')}
                        </span>
                        <div
                            class="w-5 h-5 rounded bor flex items-center justify-center"
                            title="选择删除"
                        >
                    ...
                    <Show when={messageList().length}>
                        <div
                            class="preview-list my-2 flex-1 overflow-y-auto"
                            ref={setPreviewListRef}
                        >
                            <Show when={isMjWorking()}>
                                <div class="px-4 py-2">
                                    <div
                                        class={`noactive w-full rounded-lg ${
                                            activeIndex() == -1 ? 'active' : ''
                                        }`}
                                    >
                                        <div
                                            class="w-full rounded-lg overflow-hidden flex relative cursor-pointer left-item"
                                        ...
                                        >
                                            <div class="w-full flex items-center justify-center flex-col py-4">
                                                <img
                                                    ...
                                                    src="/svg/loading.svg"
                                                />
                                                <div class="text text-center text-xs pt-2">
                                                    加载中{drawingProgress()}...
                                                </div>
                                            </div>
                                        </div>
                                                ...
                                                >
                                                    <div class="w-full flex items-center justify-center flex-col py-4">
                                                        <img
                                                            ...
                                                            class="w-1/3"
                                                            src="https://b1.beisheng.com/common/starchain_self_image/2307/11/draw-error.png"
                                                        />
                                                        <div class="text px-4 text-center text-xs pt-2 leading-relaxed">
                                                            {
                        ...
                        <div class="flex-1 flex items-center justify-center flex-col">
                            <div class="not-record">
                                <img
                                    alt=""
                                    class="img w-1/2"
                                    src="/images/draw-left-empty.png"
                                />
                                <img
                                    alt=""
                                    class="img1 w-1/2"
                                    src="/images/draw-left-empty1.png"
                                />
                                    ...
                                    <div class="info-empty flex flex-col items-center justify-center rounded-lg w-3/4 h-3/4 relative">
                                        <div class="pageLoading">
                                            <img
                                                alt=""
                                                class="center-img"
                                                src="https://b1.beisheng.com/common/starchain_self_image/2307/11/drawing.gif"
                                            />
                                            <div class="num text-xs">
                                                正在绘画中{drawingProgress()}...
                                            </div>
                                            <div class="loding">
                                                <div
                                                    class="bar"
                                        ...
                                        <div
                                            class={`tips flex justify-center items-end w-full h-full absolute text-center text-base transition-opacity duration-500 ease-in-out ${
                                                isAssistantHintVisible()
                                                    ? 'opacity-100'
                                                    : 'opacity-0'
                                            }`}
                                        >
                                    ...
                                    <div class="right w-1/3 max-w-lg rounded-2xl px-4 flex flex-col overflow-y-auto">
                                        <div class="top flex py-4 items-end justify-between">
                                            <div class="text text-lg font-semibold">
                                                {' '}
                                                {i18n.t('englishKeywords')}{' '}
                                                <i
                                                    class={`iconfont ${
                                                        showEnPromptCopy()
                                                            ? 'icon-fuzhi cursor-pointer'
                                                            : 'icon-duihao active'
                                                    } text-lg mr-1`}
                                                    onClick={async () => {
                                                        setShowEnPromptCopy(
                        ...
                        几秒前
                                        ...
                                        <div class="top flex py-4 items-end justify-between">
                                            <div class="text text-lg font-semibold">
                                                ...
                                                <i
                                                    class={`iconfont ${
                                                        showOriginPromptCopy()
                                                            ? 'icon-fuzhi cursor-pointer'
                                                            : 'icon-duihao active'
                                                    } text-lg mr-1`}
                                                    onClick={async () => {
                                                        setShowOriginPromptCopy(
                                                            false
                                                        );
                                                        await copyToClipboard(
                                                            mjWorkingPrompt()
                                        ...
                                        <div
                                            class="task_id flex-1 flex flex-col items-center justify-end py-3 text-sm"
                                            style="display:none;"
                                        >
                                ...
                                <div class="info-empty flex flex-col items-center justify-center rounded-lg w-3/4 h-3/4">
                                    <div class="w-40">
                                        <img
                                            alt=""
                                            class="w-full"
                                            src="https://b1.beisheng.com/common/starchain_self_image/2307/11/draw-center-empty-4f4ca3ef.png"
                                        />
                                    </div>
                                    <div class="text text-xs text-center tracking-wider pt-6">
                                        <div>{i18n.t('exploreAIWorld')}</div>
                                    </div>
                                </div>
                            </Show>
                                ...
                                <div class="left flex-1 h-full flex p-6">
                                    <Show when={!showErrorHint()}>
                                        <div
                                            ...
                                            class="left flex-1 rounded-2xl flex items-center justify-center overflow-hidden mr-2 relative"
                                        >
                                            <Show when={messageList().length}>
                                                <div
                                                    ref={setLargeImageRef}
                                                    class="el-image rounded-xl mx-auto"
                                                    onClick={async () => {
                                                        if (
                                                            !showImageViewer()
                                                        ) {
                                                            ...
                                                            if (
                                                                messageList()[
                                                                    activeIndex()
                                                                ].originImageUrl.includes(
                                                                    'discord'
                                                                )
                                                            ) {
                                                                let result =
                                                                    await queryMJCloudUrl(
                                                                        messageList()[
                                                                            activeIndex()
                                                                        ]
                                                                            .messageId
                        ...
                        2天前
                                        ...
                                        <Show when={type() == 1}>
                                            ...
                                            <div class="cell flex justify-between mb-4 h-10">
                                                <div
                                                    class="text h-10 text-base flex items-center justify-center rounded-xl cursor-pointer"
                                                    onClick={() =>
                                                        upscaling('U1')
                                                    }
                                                >
                                            ...
                                            <div class="cell flex justify-between mb-4 h-10">
                                                <div
                                                    class="text text-base flex items-center justify-center rounded-xl cursor-pointer h-10"
                                                    onClick={() =>
                                                        upscaling('V1')
                                                    }
                                                >
                                        ...
                                        <div class="cell flex justify-between mb-4 h-10">
                                            <div
                                                class="new_text h-10 text-base flex items-center justify-center rounded-xl cursor-pointer"
                                                style="display:none;"
                                            >
                                                <i class="iconfont  icon-guanjiancixinxi-shuaxin span text-lg"></i>
                                                <span class="span pl-1 text-base">
                                                    {i18n.t('regenerate')}
                                                </span>
                                            </div>
                                            <div
                                                class="del_text text-base flex items-center justify-center rounded-xl h-10 cursor-pointer"
                                                onClick={() => {
                                                    deleteMj();
                                                }}
                                            >
                                            ...
                                            <div
                                                class="split_text h-10 text-base flex items-center justify-center rounded-xl cursor-pointer"
                                                style="display:none;"
                                            >
                                        ...
                                        <div
                                            class="task_id flex-1 flex flex-col items-center justify-end py-3 text-sm hidden"
                                            style="display:none;"
                                        >
                                            任务ID：1110897820283572984
                                        </div>
                                ...
                                <div class="send-div flex items-center">
                                    <div class="el-textarea flex-1">
                                        <textarea
                                            ...
                                            autocomplete="off"
                                            placeholder={`${i18n.t(
                                                'enterDrawingDescription'
                                            )}${
                                                isMobile()
                                                    ? ''
                                                    : `，${i18n.t(
                                                          'enterDrawingDescriptionDetailedEnd'
                                                      )}`
                                            }`}
                                            id="input"
                                    ...
                                    <div
                                        class="send-btn h-10 w-36 rounded-full mr-2 flex items-center justify-center cursor-pointer"
                                        onClick={() => {
                                    ...
                                    <div
                                        class="send-btn1 h-10 w-20 rounded-full mr-2 flex items-center justify-center cursor-pointer"
                    ...
                    <div class="right-div flex-1 rounded pl-4 overflow-hidden flex flex-col">
                        ...
                        <div class="flex-1 overflow-x-hidden overflow-y-auto flex flex-col pr-4">
                            <div class="upload rounded-xl p-2 flex justify-between items-center">
                                <div
                                    class="flex-1 flex overflow-x-auto py-2 mr-2"
                                    style="display: none;"
                                ></div>
                                    ...
                                    >
                                        <For each={uploadedImageUrls()}>
                                            {(url, index) => {
                                                ...
                                                return (
                                                    <div
                                                        class="w-16 h-16 rounded-xl relative mr-2 flex-shrink-0 cursor-pointer"
                                                        onClick={() => {
                                                            setIsDeletable(
                                                                !isDeletable()
                                                            );
                                                    ...
                                                    >
                                                        ...
                                                        >
                                                            <img
                                                                class="absolute w-4 h-4 -right-0.5 -top-0.5"
                                                                src="/images/del.png"
                                                                onClick={() => {
                                                                    setUploadedImageUrls(
                                                                        [
                                                                            ...uploadedImageUrls().slice(
                                                                                0,
                                                                                index()
                                                                            ),
                                                                            ...uploadedImageUrls().slice(
                            ...
                            <div class="size flex flex-wrap justify-between">
                                <For each={picOptions}>
                                    {(option, index) => (
                                        <div
                                            class={`size-item flex-shrink-0 flex flex-col items-center mb-2 rounded-lg py-2 cursor-pointer ${
                                                picRatioIndex() === index()
                                                    ? 'active'
                                                    : ''
                                            }`}
                                        ...
                                        >
                                            <Show when={index() < 5}>
                                                ...
                                                <div class="text1 text-xs font-normal">
                                                    {option.desc}
                                                </div>
                                            </Show>
                                            <Show when={index() === 5}>
                                                ...
                                                <div class="flex flex-1 text items-center justify-center pb-2">
                                                    <input
                                                        ref={setRatioWidthRef}
                                                        class="input w-7 text-center appearance-none text-base"
                                                        type="number"
                                                    />{' '}
                                                    :{' '}
                                                    <input
                                                        ref={setRatioHeightRef}
                                                        class="input w-7 text-center appearance-none text-base"
                                                        type="number"
                                                    />
                                                </div>
                            ...
                            <div class="keywords-nav py-1 px-2 flex rounded-lg mb-2">
                                <div class="items-item flex-1 h-8 flex relative">
                                    <div
                                        class="item_bg rounded-lg absolute h-full w-1/6"
                                        style={`left: ${
                                            (keywordsIndex() / 6) * 100
                                        }%;`}
                                    ></div>
                                    <For each={keyWords}>
                                        {(keyWordItem, index) => (
                                            <div
                                                class="item flex-1 text-sm flex items-center justify-center cursor-pointer relative"
                                                onClick={() => {
                                    ...
                                    {(keyWordTypeInfo, index) => {
                                        ...
                                        return (
                                            <>
                                                ...
                                                >
                                                    <i
                                                        class={`iconfont  icon-xiangxia keywords-icon text-sm ${
                                                            expanded()
                                                                ? 'expansion_icon'
                                                                : ''
                                                        }`}
                                                    ...
                                                    <div class="flex flex-wrap text-sm cursor-pointer">
                                                        ...
                                                        >
                                                            {(
                                                                ...
                                                                return (
                                                                    <div
                                                                        class={`flex-shrink-0 flex mr-2 mb-3 rounded-md overflow-hidden ${
                                                                            highlighted()
                                                                                ? 'highlight'
                                                                                : ''
                                                                        }`}
